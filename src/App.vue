<template>
<v-app>
    <v-navigation-drawer v-model="leftDrawer" app clipped>
        <v-img class='mx-3 my-3' src='http://b-ssl.duitang.com/uploads/item/201511/13/20151113110434_kyReJ.jpeg'></v-img>
        <v-row class='text-center'>
            <v-col><span>sad</span></v-col>
            <v-col><span>sad</span></v-col>
            <v-col><span>sad</span></v-col>
        </v-row>
        <v-list>
            <v-subheader>MENU</v-subheader>
            <v-list-item-group v-model="item" color="teal">
                <v-list-item v-for="(item, i) in items" :key="i" @click='routerChange(item.text)'>
                    <v-list-item-icon>
                        <v-icon v-text="item.icon"></v-icon>
                    </v-list-item-icon>
                    <v-list-item-content>
                        <v-list-item-title v-text="item.text"></v-list-item-title>
                    </v-list-item-content>
                </v-list-item>
            </v-list-item-group>
        </v-list>
    </v-navigation-drawer>
    <v-navigation-drawer v-model="rightDrawer" app dark right>
        <v-img src='http://b-ssl.duitang.com/uploads/item/201511/13/20151113110434_kyReJ.jpeg'></v-img>
    </v-navigation-drawer>
    <v-app-bar app dark clipped-left>
        <v-app-bar-nav-icon @click="leftDrawer = !leftDrawer" />
        <v-toolbar-title>Teotihuacan</v-toolbar-title>
        <v-spacer></v-spacer>
        <v-text-field solo-inverted rounded hide-details prepend-inner-icon="mdi-magnify" label="Search" />
        <v-badge color='teal' overlap>
            <template v-slot:badge>0</template>
            <v-icon>mdi-bell</v-icon>
        </v-badge>
        <v-btn icon>
            <v-icon>mdi-apps</v-icon>
        </v-btn>
        <v-avatar tile @click="rightDrawer = !rightDrawer">
            <v-img src='http://b-ssl.duitang.com/uploads/item/201511/13/20151113110434_kyReJ.jpeg'></v-img>
        </v-avatar>
    </v-app-bar>
    <v-content dark>
        <router-view></router-view>
    </v-content>
    <v-footer app color="teal darken-1" dark>
        <v-spacer></v-spacer>
        <div>&copy; {{ new Date().getFullYear() }}</div>
    </v-footer>
</v-app>
</template>

<script>
export default {
    data: () => ({
        leftDrawer: true,
        rightDrawer: false,
        item: 1,
        items: [{
                text: 'Toys',
                icon: 'mdi-clock'
            },
            {
                text: 'Idoles',
                icon: 'mdi-account'
            },
            {
                text: 'Blogs',
                icon: 'mdi-flag'
            },
            {
                text: 'Music',
                icon: 'mdi-camera'
            }, 
            {
                text: 'Gallery',
                icon: 'mdi-camera'
            }
        ],
    }),
    methods:{
        routerChange(routeName){
            //为啥是个这个符号
            this.$router.push({path:`/${routeName}`});
        }
    }
}
</script>
